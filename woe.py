...
class WOE:
    ...
    def woe_df(self, df, columns, target, is_vi =True):
        try:
            X = df[columns].values
        except:
            raise KeyError + ' columns key err'
        try:
            y = df[target].values
        except:
            raise KeyError + ' columns key err'
        ...
        if is_vi:
            woe = vi.copy()
        series = pd.Series(np.zeros(len(df)))  # 局部变量用于赋值
        for i in range(len(columns)):
            for k, v in woe[i].items():
                series[df[columns[i]] == k] = v
            df[columns[i]] = series
        return df
...
a = WOE()
b = a.woe_df(df, ['SEX', 'EDUCATION'], 'target')